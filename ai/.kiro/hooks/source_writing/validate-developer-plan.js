...
function block(reason) {
  console.error(
    "BLOCKED: developer requires an approved .plan/<task-name>/task.md before using this tool.",
  )
...
if (!fs.existsSync(marker)) {
  block(
    `Write the active plan folder path to ${marker} before delegating to developer.`,
  )
}
...
if (path.dirname(activePlan) !== planRoot) {
  block(
    `Active developer plan must point inside .plan as a direct task folder: ${activePlan}`,
  )
}
