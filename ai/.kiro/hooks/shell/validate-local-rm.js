...
function tokenize(command) {
  ...
  for (let i = 0; i < command.length; i += 1) {
    ...
    if (!quote && /[;&|<>()`$]/.test(char)) {
      block(
        "Use simple literal rm targets only; dynamic shell syntax is not allowed.",
      )
    }
    ...
    if (char === "\\" && quote !== "'") {
      i += 1
      if (i >= command.length)
        block("rm command has an unfinished escape sequence.")
      token += command[i]
      continue
    }
...
for (const target of targets) {
  if (/[?*[{]/.test(target)) {
    block(
      `Use simple literal rm targets only; dynamic shell syntax is not allowed: ${target}`,
    )
  }
