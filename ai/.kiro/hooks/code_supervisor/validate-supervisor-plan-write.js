...
function activePlanFromContent(content, repoRoot, planRoot) {
  ...
  if (!activePlan) {
    block(
      "BLOCKED: Could not determine active developer plan path from write content.",
    )
  }
...
function validatePlannerReadyPlan(activePlan, planRoot) {
  if (path.dirname(activePlan) !== planRoot) {
    block(
      `BLOCKED: Active developer plan must point inside .plan as a direct task folder: ${activePlan}`,
    )
  }
  ...
  if (!fs.existsSync(taskFile)) {
    block(
      `BLOCKED: task.md was not found at ${taskFile}. Delegate planner before activating this plan.`,
    )
  }
  if (!fs.existsSync(questionsFile)) {
    block(
      `BLOCKED: questions.md was not found at ${questionsFile}. Delegate planner before activating this plan.`,
    )
  }
  if (fs.readFileSync(questionsFile, "utf8").trim() !== "NO_QUESTIONS") {
    block(
      "BLOCKED: Active developer plan requires questions.md to be exactly NO_QUESTIONS.",
    )
  }
  if (!fs.existsSync(markerFile)) {
    block(
      `BLOCKED: ${markerFile} was not found. Delegate planner to mark this plan ready before activation.`,
    )
  }
}
...
if (!isUnderOrEqual(absolutePath, planRoot)) {
  block(
    `BLOCKED: code_supervisor cannot write to '${absolutePath}'. Only .plan/ is writable.`,
  )
}
...
if (
  parts.length === 2 &&
  ["task.md", "questions.md", ".planner-ready.json"].includes(filename)
) {
  block(
    `BLOCKED: ${filename} is planner-owned. Delegate planner instead of writing it from code_supervisor.`,
  )
}
