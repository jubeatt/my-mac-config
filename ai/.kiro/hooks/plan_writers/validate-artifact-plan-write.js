...
if (typeof requestedPath !== "string" || requestedPath.length === 0) {
  block(
    "BLOCKED: artifact writer cannot write to an unknown path.\nOnly .plan/ is writable.",
  )
}
...
if (!isUnderOrEqual(absolutePath, planRoot)) {
  block(
    `BLOCKED: artifact writer cannot write to '${absolutePath}'.\nOnly .plan/ is writable.`,
  )
}
...
if (
  ...
  block(
    `BLOCKED: ${filename} is planner-owned. Use planner for plan creation or revision.`,
  )
}
